...
class DT:
    ...
    def terminal_node_output(self, targets):
        return [self.count(targets, i) / len(targets) for i in range(2)]
    ...
    def buildTree(self, inputs, targets, node, depth, tree):
        ...
        if (depth >= self.max_depth or entropy_val <= self.min_entropy or len(targets) <= self.min_elem):
            ...
            for i in range(inputs.shape[1]):
                if i < 9:
                    l_r_ind = self.split_set(inputs[:, i], 0.5)
                    if len(l_r_ind[0]) != 0 and len(l_r_ind[1]) != 0:
                        split_ind.append(i)
                        split_val.append(0.5)
                    ...
                    psi_inp = inputs[:, i]
                    min_v = np.amin(psi_inp, axis=0)
                    max_v = np.amax(psi_inp, axis=0)
                    step = (max_v - min_v) / 10
                    thau = np.arange(min_v, max_v, step)
                    for t in thau:
            ...
            node.depth = depth
            tree.grow_branches(node)
            self.buildTree(inputs_left[argmaxI], targets_left[argmaxI], node.left, depth + 1, tree)
            self.buildTree(inputs_right[argmaxI], targets_right[argmaxI], node.right, depth + 1, tree)
